...
export default function ReviewsClient({ initialReviews, initialStats }: ReviewsClientProps) {
  ...
  // Data state — arranca con datos del servidor (SSR)
  ...
  // Fetch reviews cuando cambia el filtro — el render inicial usa datos del servidor
  useEffect(() => {
    if (!mounted.current) {
    ...
    Promise.all([
      fetch(`${API_BASE}/api/v1/reviews?status=APPROVED&stars=${activeFilter}`).then((r) => r.json()),
      ...
      .catch(() => { });
  }, [activeFilter, initialReviews, initialStats]);
