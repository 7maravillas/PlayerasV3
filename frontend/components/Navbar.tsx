...
// --- MENSAJES DEL CARRUSEL (base, se completan dinámicamente) ---
...
const Navbar = () => {
  ...
  const [announcements, setAnnouncements] = useState(BASE_ANNOUNCEMENTS);
  ...
  // Fetch cupón activo para el carrusel
  useEffect(() => {
    fetch(`${API_BASE}/api/v1/coupons/announcement`)
      .then((r) => r.json())
      .then((data) => {
        if (data?.code) {
          setAnnouncements([
            "ENVÍOS GRATIS a partir de $999 MXN",
            "AUTENTICIDAD GARANTIZADA • CALIDAD PREMIUM",
            `${data.discountPercent}% DE DESCUENTO EN TU PRIMERA COMPRA • CÓDIGO: ${data.code}`,
            "PAGO SEGURO • ENCRIPTACIÓN SSL",
          ]);
        }
      })
                ...
                {isOpen && (
                  <div className="bg-gray-50">
                    ...
                    {DROPDOWN_SUBCATEGORIES.map((sub) => (
                      <Link
                        key={sub.label}
                        href={`/catalog?audience=${audience}${sub.tagFilter ? `&tags=${sub.tagFilter}` : ''}${sub.sort ? `&sort=${sub.sort}` : ''}`}
                        onClick={() => setDrawerOpen(false)}
